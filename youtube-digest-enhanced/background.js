...
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
  ...
  if (message.action === "generateTldr") {
    handleGenerateTldr(
      message.transcriptText,
      message.videoTitle,
      message.channelName,
...
/**
 * Generates a standalone TL;DR summary using DeepSeek.
 * Asks for a 2-3 sentence summary of the video.
 *
 * @param {string} transcriptText - The full transcript text
 * @param {string} videoTitle - The video title
 * @param {string} channelName - The channel name
 * @returns {Object} - { success, tldr } or { success: false, error }
 */
async function handleGenerateTldr(transcriptText, videoTitle, channelName) {
  try {
    const settings = await getSettings();
    if (!settings.aiApiKey) {
      return {
      ...
      messages: [
        {
          role: "system",
          content:
            "You are a helpful assistant. Summarize the video transcript in 2-3 sentences. " +
            'Return JSON: {"tldr": "your summary here"}. Output only valid JSON.',
        },
        {
          role: "user",
    ...
    const tldr = (parsed.tldr || "").trim().slice(0, 1000);

    return { success: true, tldr };
  } catch (error) {
    console.error("TL;DR generation error:", error);
    return { success: false, error: error.message || "Failed to generate TL;DR" };
  }
}
...
function validateAndFixTimestamps(analysis, maxSeconds) {
  ...
  // TL;DR — pass through as a sanitized string
  const tldr = safeString(analysis?.tldr, 1000);

  // Key Terms — sanitize and validate structure
  const keyTerms = (
    Array.isArray(analysis?.keyTerms) ? analysis.keyTerms : []
  )
    .slice(0, 20)
    .map((item) => {
      const term = safeString(item?.term, 200);
      const definition = safeString(item?.definition, 1000);
      if (!term) return null;
      return { term, definition };
    })
    .filter(Boolean);

  return { chapters, keyQuotes, keyMoments, tldr, keyTerms };
}
...
// ============================================================
// TRANSLATION — Translate transcript batches into multiple languages
// ============================================================
 ...
 *
 * @param {string} targetLanguage - Language code (zh, ja, ko, es, fr, de)
 * @returns {Promise<string>} - The base translation rules
 */
async function getTranslationBaseRules(targetLanguage) {
  const langName = TRANSLATION_LANGUAGES[targetLanguage];
  if (!langName) {
  ...
  if (targetLanguage === "zh") {
    langSpecific = await loadPromptSection("translation.md", "Chinese rules");
  } else {
    langSpecific = `- Use natural ${langName} sentence structures and idioms.`;
  }
...
function looksLikeTranslation(text, sourceText, targetLanguage) {
  ...
  // Latin-script languages: translation should differ from source
  return text.trim().toLowerCase() !== sourceText.trim().toLowerCase();
}

/**
 * Aligns untrusted model output by exact stable ID. Missing, duplicated,
 * unknown, empty, or clearly invalid values become explicit row errors.
 */
function normalizeTranslatedSegmentBatch(parsed, sourceSegments, targetLanguage) {
  ...
  candidates.forEach((candidate) => {
    ...
    if (text && looksLikeTranslation(text, source.text, targetLanguage)) {
...
async function handleTranslateContent(
  ...
  try {
    ...
    const aligned = normalizeTranslatedSegmentBatch(parsed, sourceSegments, targetLanguage);
