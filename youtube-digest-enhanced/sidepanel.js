...
// Language display names for translation modes
...
// --- Translation state ---
// Supports original, multiple target languages, and bilingual (original + translation).
let currentTranscriptMode = "original";
let currentTranslationLang = "zh"; // Actual language code for translation (used by bilingual mode)
...
async function initTheme() {
  const result = await chrome.storage.local.get("ytd_theme");
  const theme = result.ytd_theme || "light";
  applyTheme(theme);
}
...
async function toggleTheme() {
  ...
  applyTheme(next);
  await chrome.storage.local.set({ ytd_theme: next });
}
...
function setupEventListeners() {
  ...
  document
    ...
    ?.addEventListener("click", exportTranscriptMarkdown);

  // Transcript search
  document
    .getElementById("transcriptSearchInput")
    ?.addEventListener("input", handleTranscriptSearch);
  document
  ...
  // Language dropdown
  document
    .getElementById("transcriptLangSelect")
    ?.addEventListener("change", (e) => {
      handleTranscriptModeChange(e.target.value);
    });
  ...
  document
    .getElementById("exportNotesMarkdownBtn")
...
function handleKeyboardShortcuts(e) {
  ...
  // Escape always works (clears search even when typing in search input)
  if (e.key === "Escape") {
    const searchInput = document.getElementById("transcriptSearchInput");
    if (searchInput && (searchInput.value || active === searchInput)) {
  ...
  switch (e.key) {
    case "/":
      e.preventDefault();
      document.getElementById("transcriptSearchInput")?.focus();
      break;
    case "j":
      e.preventDefault();
      navigateTranscript(1);
      break;
    case "k":
      e.preventDefault();
      navigateTranscript(-1);
...
/**
 * Scrolls to the next/previous transcript entry relative to the current view.
...
function navigateTranscript(direction) {
  ...
  const viewportMid = contentArea.scrollTop + contentArea.clientHeight / 2;
  ...
  for (let i = 0; i < entries.length; i++) {
    ...
    const absTop = entryMid + contentArea.scrollTop - contentArea.offsetTop;

    if (direction > 0 && absTop > viewportMid + 10) {
      currentIndex = i;
      found = true;
      break;
    }
  ...
  if (!found && direction < 0) {
    // Use the last entry above viewport mid
  }
  if (!found && direction > 0) {
...
function renderAnalysisResults(analysis) {
  ...
  if (tldrContent) {
    tldrContent.textContent = analysis.tldr || "No summary available.";
  }
  ...
  if (keyTermsList) {
    ...
    if (terms.length === 0) {
      keyTermsList.innerHTML =
        '<div class="key-terms-placeholder" style="color: var(--text-muted);">No key terms found.</div>';
    } else {
      terms.forEach((item) => {
        const card = document.createElement("div");
        card.className = "key-term-card";
        card.innerHTML = `
          <div class="key-term-name">${escapeHtml(item.term)}</div>
          <div class="key-term-def">${escapeHtml(item.definition || "")}</div>
        `;
        keyTermsList.appendChild(card);
      });
    }
  }
}
...
function handleTranscriptSearch(e) {
  ...
  const clearBtn = document.getElementById("transcriptSearchClear");
  ...
  const noResults = document.getElementById("transcriptSearchNoResults");
  ...
  if (matchCount === 0) {
    ...
    msg.id = "transcriptSearchNoResults";
    msg.className = "transcript-search-no-results";
...
function exportTranscriptMarkdown() {
  const videoUrl = `https://youtube.com/watch?v=${currentVideoId}`;
  let md = `# ${currentVideoTitle || "Untitled Video"}\n\n`;
  md += `**Channel:** ${currentChannelName || "Unknown"}\n`;
  md += `**URL:** ${videoUrl}\n\n`;
  ...
  if (currentTranscript && currentTranscript.length) {
    currentTranscript.forEach((entry) => {
      ...
      const timestamp = `${minutes}:${String(seconds).padStart(2, "0")}`;
      md += `**[${timestamp}]** ${entry.text}\n\n`;
    });
  } else {
    md += `${currentTranscriptText || "No transcript available."}\n\n`;
  }

  md += `---\n*Exported by YouTube Digest*\n`;

  const filename = `${sanitizeFilename(currentVideoTitle)}-transcript.md`;
...
async function exportNotesMarkdown() {
  try {
    ...
    const result = await chrome.runtime.sendMessage({
      action: "getNotes",
      videoId: filterAll ? null : currentVideoId,
    });
    ...
    let md = `# YouTube Digest Notes\n\n`;
    if (filterAll) {
      md += `*All notes*\n\n`;
    } else {
      md += `**Video:** ${currentVideoTitle || "Unknown"}\n`;
      md += `**Channel:** ${currentChannelName || "Unknown"}\n\n`;
    }

    result.notes.forEach((note, index) => {
      md += `## Note ${index + 1} — [${note.timestamp}](${note.timestampedUrl})\n\n`;
      if (filterAll && note.videoTitle) {
        md += `**Video:** ${note.videoTitle}\n\n`;
      }
      md += `${note.text}\n\n`;
    });
    ...
    const filename = filterAll
      ? "youtube-digest-all-notes.md"
      : `${sanitizeFilename(currentVideoTitle)}-notes.md`;
    downloadMarkdownFile(md, filename);
  } catch (error) {
...
async function triggerAnalysis() {
  ...
  if (tldrContent) tldrContent.textContent = "Loading summary...";
  if (chapterList)
    chapterList.innerHTML =
  ...
  if (keyTermsList)
    keyTermsList.innerHTML =
      '<div class="key-terms-placeholder" style="color: var(--text-muted);">Loading key terms...</div>';
...
function getActiveTranslationLang() {
  if (currentTranscriptMode === "bilingual") return currentTranslationLang;
  return currentTranscriptMode;
}

function transcriptTranslationCacheKey(segment) {
  return `${currentVideoId}:${getActiveTranslationLang()}:semantic:${segment.id}`;
}

function setTranscriptModeUI(mode) {
  // Update the dropdown
  const select = document.getElementById("transcriptLangSelect");
  if (select) select.value = mode;
  // Update any legacy mode buttons
...
async function handleTranscriptModeChange(mode) {
  const validModes = ["original", "zh", "bilingual", "ja", "ko", "es", "fr", "de"];
  if (!validModes.includes(mode)) return;
  if (mode === currentTranscriptMode) return;

  // Track the actual translation language for bilingual mode
  if (mode !== "original" && mode !== "bilingual") {
    currentTranslationLang = mode;
  }
...
function renderTranscriptModeRows(segments, mode) {
  ...
  const langName = TRANSLATION_LANGUAGE_NAMES[getActiveTranslationLang()] || "Translation";
  const modeLabel =
    mode === "bilingual"
      ? `${originalLabel} + ${langName}`
      : `${langName} · translated from ${originalLabel}`;
...
async function requestTranscriptTranslationBatch(
  ...
  try {
    const result = await sendTranslationMessage({
      ...
      targetLanguage: getActiveTranslationLang(),
      videoTitle: currentVideoTitle,
    });
