...
test("Transcript header exposes a language dropdown with multi-language support", () => {
  const html = read("sidepanel.html");
  const js = read("sidepanel.js");
  assert.match(html, /<select[^>]*id="transcriptLangSelect"/);
  assert.match(html, /<option value="original">Original</);
  assert.match(html, /<option value="zh">/);
  ...
  assert.match(js, /handleTranscriptModeChange\(e\.target\.value\)/);
...
test("background rejects unsupported language fallthrough and malformed batches", () => {
  ...
  assert.match(source, /TRANSLATION_LANGUAGES/);
  assert.throws(
